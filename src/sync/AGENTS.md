...
| `folder.config_changed` | `{"notebookId":"<id>","path":"<rel-folder-path>"}` | `BundledFolderManager::SaveFolderConfig` (`bundled_folder_manager.cpp:256`), in the success branch of the vx.json write. | Any write to a folder's vx.json, including structural ops (rename, move, copy, import) that have no dedicated event of their own. `SyncManager::mark_dirty` subscribes here for auto-sync wakeup. |
| `notebook.config_changed` | `{"notebookId":"<id>"}` | `BundledNotebook::UpdateConfig` (`bundled_notebook.cpp:193-196`), in the success branch of the vx_notebook/config.json write. Guarded by `event_manager_ != nullptr`. | Any write to the notebook's config.json. The nullptr guard suppresses emission during `NotebookManager::CreateNotebook` before `SetEventManager` is wired (T2 plumbing at `notebook_manager.cpp:125`). `SyncManager::mark_dirty` subscribes here for auto-sync wakeup. |
| `file.tagged` | `{"notebookId":"<id>","path":"<rel-file-path>","tag":"<name>"}` | `BundledFolderManager::TagFile` (`bundled_folder_manager.cpp:1231`), after the tag is appended and SaveFolderConfig succeeds. | Single tag added to a file via the dedicated tag API. UI-only consumer; `mark_dirty` does NOT subscribe (the paired `folder.config_changed` drives auto-sync). |
| `file.untagged` | `{"notebookId":"<id>","path":"<rel-file-path>","tag":"<name>"}` | `BundledFolderManager::UntagFile` (`bundled_folder_manager.cpp:1274`), after the tag is removed and SaveFolderConfig succeeds. | Single tag removed from a file. UI-only consumer. |
| `file.tags_replaced` | `{"notebookId":"<id>","path":"<rel-file-path>","tags":["<tag1>", ...]}` | `BundledFolderManager::UpdateFileTags` (`bundled_folder_manager.cpp:1179`), after the new tags array is written and SaveFolderConfig succeeds. | Full tag-array replacement on a file. UI-only consumer. |
| `file.metadata_updated` | `{"notebookId":"<id>","path":"<rel-file-path>"}` | `BundledFolderManager::UpdateFileMetadata` (`bundled_folder_manager.cpp:1124`), after the metadata patch is merged and SaveFolderConfig succeeds. | Metadata-only file update (no content change, no rename). UI-only consumer. |
| `folder.metadata_updated` | `{"notebookId":"<id>","path":"<rel-folder-path>"}` | `BundledFolderManager::UpdateFolderMetadata` (`bundled_folder_manager.cpp:483`), after the metadata patch is merged and SaveFolderConfig succeeds. | Metadata-only folder update. UI-only consumer. |
| `file.attached` | `{"notebookId":"<id>","path":"<rel-file-path>","attachment":"<name>"}` | `BundledFolderManager::AddFileAttachment` (`bundled_folder_manager.cpp:2777`), after the attachment is appended and SaveFolderConfig succeeds. | Single attachment added to a file. Idempotent on duplicate: no emit if the attachment was already present. UI-only consumer. |
| `file.detached` | `{"notebookId":"<id>","path":"<rel-file-path>","attachment":"<name>"}` | `BundledFolderManager::DeleteFileAttachment` (`bundled_folder_manager.cpp:2823`), after the attachment is removed and SaveFolderConfig succeeds. | Single attachment removed from a file. Idempotent on missing: no emit if the attachment was already absent. UI-only consumer. |
| `file.attachments_replaced` | `{"notebookId":"<id>","path":"<rel-file-path>","attachments":["<name1>", ...]}` | `BundledFolderManager::UpdateFileAttachments` (`bundled_folder_manager.cpp:2728`), after the new attachments array is written and SaveFolderConfig succeeds. | Full attachment-array replacement on a file. UI-only consumer. |

The two persistence events (`folder.config_changed`, `notebook.config_changed`) are subscribed by `SyncManager::mark_dirty` and drive auto-sync wakeup. Any write to a folder's vx.json or to the notebook's config.json flags the notebook dirty regardless of which higher-level mutation triggered it; this is what makes structural ops like RenameFolder / MoveFolder / CopyFolder reach auto-sync without a dedicated structural event of their own (proven by `test_rename_folder_triggers_sync_chain` in `tests/test_sync.cpp`).

The eight semantic events (`file.tagged`, `file.untagged`, `file.tags_replaced`, `file.metadata_updated`, `folder.metadata_updated`, `file.attached`, `file.detached`, `file.attachments_replaced`) are NOT subscribed by `mark_dirty`. They exist as a richer payload surface for UI consumers and future Qt `EventBridge` integration; the `folder.config_changed` fired by the same mutation handles the sync wakeup half of the contract, so adding `mark_dirty` subscriptions to the semantic events would double-fire `sync.should_run`.
